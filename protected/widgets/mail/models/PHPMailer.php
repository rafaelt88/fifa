...
class PHPMailer {
 ...
 public function init() {}      
  ...
  protected function AddAnAddress($kind, $address, $name = '') {
    ...
    $name = trim(preg_replace('/[\r\n]+/', null, $name)); //Strip breaks and trim
 ...
 * @return boolean
 */
  public function SetFrom($address, $name = null, $auto = 1) {
    $address = trim($address);
    $name = trim(preg_replace('/[\r\n]+/', null, $name)); //Strip breaks and trim
    if (!$this->ValidateAddress($address)) {
      $this->SetError($this->Lang('invalid_address').': '. $address);
  ...
  protected function SmtpSend($header, $body) {
    ...
    foreach($this->to as $to) {
      if (!$this->smtp->Recipient($to[0])) {
        ...
        // implement call back function if it exists
        $isSent = 0;
        $this->doCallback($isSent, $to[0], null, null, $this->Subject, $body);
      } else {
        // implement call back function if it exists
        $isSent = 1;
        $this->doCallback($isSent, $to[0], null, null, $this->Subject, $body);
    ...
    foreach($this->cc as $cc) {
      if (!$this->smtp->Recipient($cc[0])) {
        ...
        // implement call back function if it exists
        $isSent = 0;
        $this->doCallback($isSent, null, $cc[0], null, $this->Subject, $body);
      } else {
        // implement call back function if it exists
        $isSent = 1;
        $this->doCallback($isSent, null, $cc[0], null, $this->Subject, $body);
    ...
    foreach($this->bcc as $bcc) {
      if (!$this->smtp->Recipient($bcc[0])) {
        ...
        // implement call back function if it exists
        $isSent = 0;
        $this->doCallback($isSent, null, null, $bcc[0], $this->Subject, $body);
      } else {
        // implement call back function if it exists
        $isSent = 1;
        $this->doCallback($isSent, null, null, $bcc[0], $this->Subject, $body);
  ...
  */
  ...
  public function CreateBody() {
    ...
    switch($this->message_type) {
      case 'inline':
        $body .= $this->GetBoundary($this->boundary[1], null, null, '');
      ...
      case 'inline_attach':
        ...
        $body .= $this->LE;
        $body .= $this->GetBoundary($this->boundary[2], null, null, '');
        $body .= $this->EncodeString($this->Body, $this->Encoding);
        $body .= $this->LE.$this->LE;
        ...
        break;
      case 'alt':
        $body .= $this->GetBoundary($this->boundary[1], null, 'text/plain', '');
        $body .= $this->EncodeString($this->AltBody, $this->Encoding);
        $body .= $this->LE.$this->LE;
        $body .= $this->GetBoundary($this->boundary[1], null, 'text/html', '');
        $body .= $this->EncodeString($this->Body, $this->Encoding);
        $body .= $this->LE.$this->LE;
      ...
      case 'alt_inline':
        ...
        $body .= $this->TextLine("\tboundary=\"" . $this->boundary[2] . '"');
        $body .= $this->LE;
        $body .= $this->GetBoundary($this->boundary[2], null, 'text/html', '');
        $body .= $this->EncodeString($this->Body, $this->Encoding);
        $body .= $this->LE.$this->LE;
        $body .= $this->AttachAll("inline", $this->boundary[2]);
        $body .= $this->LE;
      ...
      case 'alt_attach':
        ...
        $body .= $this->TextLine("\tboundary=\"" . $this->boundary[2] . '"');
        $body .= $this->LE;
        $body .= $this->GetBoundary($this->boundary[2], null, 'text/plain', '');
        $body .= $this->EncodeString($this->AltBody, $this->Encoding);
        $body .= $this->LE.$this->LE;
        $body .= $this->GetBoundary($this->boundary[2], null, 'text/html', '');
        $body .= $this->EncodeString($this->Body, $this->Encoding);
      ...
      case 'alt_inline_attach':
        ...
        $body .= $this->LE;
        $body .= $this->GetBoundary($this->boundary[3], null, 'text/html', '');
        $body .= $this->EncodeString($this->Body, $this->Encoding);
        $body .= $this->LE.$this->LE;
   ...
   * @return bool
   */
  public function AddAttachment($path, $name = null, $encoding = 'base64', $type = 'application/octet-stream') {
    try {
      if ( !@is_file($path) ) {
  ...
  * @return string
  */
  public function EncodeQPphp( $input = null, $line_max = 76, $space_conv = false) {
  ...
  */
   ...
   * @return bool
   */
  public function AddEmbeddedImage($path, $cid, $name = null, $encoding = 'base64', $type = 'application/octet-stream') {
   ...
   * @return bool
   */
  public function AddStringEmbeddedImage($string, $cid, $name = null, $encoding = 'base64', $type = 'application/octet-stream') {
    // Append to $attachment array
    $this->attachment[] = array(
      0 => $string,
      1 => $name,
  ...
  public function MsgHTML($message, $basedir = '') {
    ...
    $this->Body = $message;
    if (empty($this->AltBody)) {
        $textMsg = trim(strip_tags(preg_replace('/<(head|title|style|script)[^>]*>.*?<\/\\1>/s', null, $message)));
        if (!empty($textMsg)) {
